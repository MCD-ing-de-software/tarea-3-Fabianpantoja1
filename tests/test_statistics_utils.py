...
class TestStatisticsUtils(unittest.TestCase):
    ...
    def test_moving_average_basic_case(self):
        ...
        para comparar arrays de NumPy - esto es mejor que unittest porque maneja la comparación de arrays numéricos con tolerancia 
        para errores de punto flotante)
        - Verificar que el resultado tiene la forma (shape) esperada (usar self.assertEqual para comparar tuplas de .shape - 
        ...
        # Comparación de arrays con tolerancia para errores de precisión
        npt.assert_allclose(result, expected, rtol=1e-7, atol=1e-7)
    ...
    def test_moving_average_raises_for_invalid_window(self):
        ...
        # Verifica window=0 arroja ValueError
        with self.assertRaises(ValueError):
            utils.moving_average(arr, window=0)
        
        # Verifica que window mayor que len(arr) lanza ValueError
        with self.assertRaises(ValueError):
    ...
    def test_moving_average_only_accepts_1d_sequences(self):
        ...
        # Asegurar que no acepte arrays 2D debe dar ValueError
        with self.assertRaises(ValueError):
    ...
    def test_zscore_has_mean_zero_and_unit_std(self):
        ...
        - Verificar que la media del resultado es aproximadamente 0 (usar self.assertAlmostEqual para un solo valor numérico 
        - unittest es suficiente)
        - Verificar que la desviación estándar del resultado es aproximadamente 1 (usar self.assertAlmostEqual para un solo 
        ...
        # Validar que el promedio tienda a cero
        mean = np.mean(result)
        self.assertAlmostEqual(mean, 0.0, places=10)

        # Validar que el promedio tienda a uno
        std = np.std(result)
    ...
    def test_zscore_raises_for_zero_std(self):
        ...
        - Llamar a zscore con esa secuencia y verificar que se lanza un ValueError indicando que la desviación estándar 
        es cero (usar self.assertRaises)"""

        utils = StatisticsUtils()
        arr = [5, 5, 5]

        # Debe fallar si no hay variabilidad (std = 0).
        with self.assertRaises(ValueError):
            utils.zscore(arr)
    ...
    def test_min_max_scale_maps_to_zero_one_range(self):
        ...
        - Verificar que los valores transformados son correctos (ej: [0.0, 0.5, 1.0] para [2, 4, 6]) (usar numpy.testing.assert_allclose() 
        para comparar el array completo - esto es necesario para comparar arrays de NumPy con tolerancia para errores de punto flotante)"""

        utils = StatisticsUtils()
        arr = [2, 4, 6]
        result = utils.min_max_scale(arr)

        # Punto de control para el límite inferior
        min_val = np.min(result)
        self.assertAlmostEqual(min_val, 0.0, places=10)

        # Punto de control para el límite superior
